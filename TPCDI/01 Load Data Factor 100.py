...
# MAGIC %sh
# MAGIC 
# MAGIC du -ch /dbfs/tmp/tpc-di/100/ | grep total
...
# MAGIC %sql 
# MAGIC 
# MAGIC create database if not exists lakehouse_tpcdi_100; 
...
# MAGIC %sh
# MAGIC 
# MAGIC du -ch /dbfs/tmp/tpc-di/100/Batch*/CustomerMgmt.xml | grep total

# COMMAND ----------

# MAGIC %sql 
# MAGIC drop table if exists lakehouse_tpcdi_100.customermgmt_raw; 
# MAGIC 
# MAGIC create table lakehouse_tpcdi_100.customermgmt_raw
# MAGIC using com.databricks.spark.xml
# MAGIC OPTIONS (path "dbfs:/tmp/tpc-di/100/Batch1/CustomerMgmt.xml", rowTag "TPCDI:Action")

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC select * from lakehouse_tpcdi_100.customermgmt_raw

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC Create or replace table lakehouse_tpcdi_100.customer_management
# MAGIC as 
# MAGIC select
...
# MAGIC from
# MAGIC   lakehouse_tpcdi_100.customermgmt_raw
# MAGIC   
# MAGIC   
...
# MAGIC %sql
# MAGIC 
# MAGIC select * from lakehouse_tpcdi_100.customer_management

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC OPTIMIZE lakehouse_tpcdi_100.customer_management ZORDER BY CA_B_ID; 
# MAGIC ANALYZE TABLE lakehouse_tpcdi_100.customer_management COMPUTE STATISTICS FOR ALL COLUMNS;

# COMMAND ----------

# MAGIC %sh
# MAGIC 
# MAGIC du -ch /dbfs/tmp/tpc-di/100/Batch1/FINWIRE[0-9][0-9][0-9][0-9]Q[1-4] | grep total

# COMMAND ----------

# MAGIC %sh head /dbfs/tmp/tpc-di/100/Batch1/FINWIRE[0-9][0-9][0-9][0-9]Q[1-4]

# COMMAND ----------

raw_finwire_df = spark.read.text('dbfs:/tmp/tpc-di/100/Batch1/FINWIRE[0-9][0-9][0-9][0-9]Q[1-4]')
...
# MAGIC %sql
# MAGIC 
# MAGIC create or replace table lakehouse_tpcdi_100.finwire_cmp_bronze as select * from finwire_cmp;
# MAGIC create or replace table lakehouse_tpcdi_100.finwire_sec_bronze as select * from finwire_sec;
# MAGIC create or replace table lakehouse_tpcdi_100.finwire_fin_bronze as select * from finwire_cmp;

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC OPTIMIZE lakehouse_tpcdi_100.finwire_cmp_bronze ZORDER BY CIK; 
# MAGIC ANALYZE TABLE lakehouse_tpcdi_100.finwire_cmp_bronze COMPUTE STATISTICS FOR ALL COLUMNS;

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC OPTIMIZE lakehouse_tpcdi_100.finwire_sec_bronze ZORDER BY CNameOrCIK; 
# MAGIC ANALYZE TABLE lakehouse_tpcdi_100.finwire_sec_bronze COMPUTE STATISTICS FOR ALL COLUMNS;

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC OPTIMIZE lakehouse_tpcdi_100.finwire_fin_bronze ZORDER BY CIK; 
# MAGIC ANALYZE TABLE lakehouse_tpcdi_100.finwire_fin_bronze COMPUTE STATISTICS FOR ALL COLUMNS;

# COMMAND ----------

# MAGIC %sql 
# MAGIC 
# MAGIC drop table if exists lakehouse_tpcdi_100.prospect_bronze

# COMMAND ----------

# MAGIC %sql 
# MAGIC 
# MAGIC CREATE OR REPLACE TABLE lakehouse_tpcdi_100.prospect_bronze
...
spark.read.format("csv") \
  ...
  .option("header", "false") \
  .option("sep", ",") \
  .load("dbfs:/tmp/tpc-di/100/Batch1/Prospect.csv") \
  .createOrReplaceTempView("prospect_raw")

# COMMAND ----------

tableSchema = spark.sql(f"select * from lakehouse_tpcdi_100.prospect_bronze limit 1").schema
...
.option("sep", ",") \
.load("dbfs:/tmp/tpc-di/100/Batch*/Prospect.csv") \
.writeStream \
.trigger(once=True) \
.format("delta") \
.option("mergeSchema", "true") \
.option("checkpointLocation", "/dbfs/tmp/tpc-di/100/checkpoints/prospect/4") \
.start("dbfs:/user/hive/warehouse/lakehouse_tpcdi_100.db/prospect_bronze") 

# COMMAND ----------

df=spark.table("lakehouse_tpcdi_100.prospect_bronze")

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC select * from lakehouse_tpcdi_100.prospect_bronze;

# COMMAND ----------

# MAGIC %sh
# MAGIC 
# MAGIC ls -l --block-size=M /dbfs/tmp/tpc-di/100/Batch*/DailyMarket.txt

# COMMAND ----------

# MAGIC %sh
# MAGIC 
# MAGIC ls /dbfs/tmp/tpc-di/100/Batch*/DailyMarket.txt

# COMMAND ----------

# MAGIC %sh
# MAGIC 
# MAGIC du -ch /dbfs/tmp/tpc-di/100/Batch*/DailyMarket.txt | grep total

# COMMAND ----------

# MAGIC %sh
# MAGIC 
# MAGIC head /dbfs/tmp/tpc-di/100/Batch1/DailyMarket.txt

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC CREATE OR REPLACE TABLE lakehouse_tpcdi_100.daily_market (
...
tableSchema = spark.sql(f"select * from lakehouse_tpcdi_100.daily_market limit 1").schema

# COMMAND ----------

spark.readStream.format("cloudFiles") \
.schema(tableSchema) \
.option("cloudFiles.schemaLocation","/dbfs/tmp/tpc-di/100/checkpoints/schemas/dailymarket2") \
.option("cloudFiles.format", "CSV") \
.option("sep", "|") \
.load("dbfs:/tmp/tpc-di/100/Batch*/DailyMarket.txt") \
.writeStream \
.trigger(once=True) \
.format("delta") \
.option("mergeSchema", "true") \
.option("checkpointLocation", "/dbfs/tmp/tpc-di/100/checkpoints/dailymarket/2") \
.start("dbfs:/user/hive/warehouse/lakehouse_tpcdi_100.db/daily_market") 
...
spark.read.format("csv") \
  ...
  .option("header", "false") \
  .option("sep", "|") \
  .load("dbfs:/tmp/tpc-di/100/Batch1/DailyMarket.txt") \
...
# MAGIC %sql 
# MAGIC 
# MAGIC insert into lakehouse_tpcdi_100.daily_market
# MAGIC select * from daily_market_raw
...
# MAGIC %sql 
# MAGIC 
# MAGIC OPTIMIZE lakehouse_tpcdi_100.daily_market ZORDER BY dm_date; 
# MAGIC ANALYZE TABLE lakehouse_tpcdi_100.daily_market COMPUTE STATISTICS FOR ALL COLUMNS;
